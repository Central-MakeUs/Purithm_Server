...
public interface UserFilterLogRepository extends JpaRepository<UserFilterLog, Long> {
	...
	@Query("SELECT f.id AS filterId, f.name AS filterName, f.thumbnail AS thumbnail, p.username AS photographer, f.membership AS membership, ufl.createdAt AS createdAt, r.id AS reviewId, f.os AS os " +
